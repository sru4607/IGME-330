...
void Simplex::MyEntityManager::UpdateCollisions(MyOctant* root) {
	...
	//For each leaf with objects check each object against the others to check if collision exists
	for each (MyOctant* child in root->m_LeafsWithObjects)
	{
		int count = child->m_EntityList.size();
		for (uint i = 0; i < count - 1; i++)
		{
			for (uint j = i + 1; j < count; j++)
			{
				int k = child->m_EntityList.at(i);
				int l = child->m_EntityList.at(j);
				m_mEntityArray[k]->IsColliding(m_mEntityArray[l]);
			}
		}
	}
	//std::cout << total;
