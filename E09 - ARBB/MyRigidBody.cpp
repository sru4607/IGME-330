...
void MyRigidBody::SetModelMatrix(matrix4 a_m4ModelMatrix)
{
	...
	vector3 cornerRealigned[8];

	////Min
	//cornerRealigned[0] = vector3(-m_v3HalfWidth.x, m_v3HalfWidth.y, -m_v3HalfWidth.z);
	//cornerRealigned[1] = vector3(m_v3HalfWidth.x, m_v3HalfWidth.y, -m_v3HalfWidth.z);
	//cornerRealigned[2] = vector3(-m_v3HalfWidth.x, m_v3HalfWidth.y, m_v3HalfWidth.z);
	//cornerRealigned[3] = vector3(m_v3HalfWidth.x, m_v3HalfWidth.y, m_v3HalfWidth.z);
	//cornerRealigned[4] = vector3(-m_v3HalfWidth.x, -m_v3HalfWidth.y, -m_v3HalfWidth.z);
	//cornerRealigned[5] = vector3(m_v3HalfWidth.x, -m_v3HalfWidth.y, -m_v3HalfWidth.z);
	//cornerRealigned[6] = vector3(-m_v3HalfWidth.x, -m_v3HalfWidth.y, m_v3HalfWidth.z);
	//cornerRealigned[7] = vector3(m_v3HalfWidth.x, -m_v3HalfWidth.y, m_v3HalfWidth.z);

	//Get corners based on OBB vectors
	//BLF
	cornerRealigned[0] = m_v3MinL;
	//BRF
	cornerRealigned[1] = vector3(m_v3MaxL.x, m_v3MinL.y, m_v3MinL.z);
	//TLF
	cornerRealigned[2] = vector3(m_v3MinL.x, m_v3MaxL.y, m_v3MinL.z);
	//TRF
	cornerRealigned[3] = vector3(m_v3MaxL.x, m_v3MaxL.y, m_v3MinL.z);
	//BLB
	cornerRealigned[4] = vector3(m_v3MinL.x, m_v3MinL.y, m_v3MaxL.z);
	//BRB
	cornerRealigned[5] = vector3(m_v3MaxL.x, m_v3MinL.y, m_v3MaxL.z);
	//TLB
	cornerRealigned[6] = vector3(m_v3MinL.x, m_v3MaxL.y, m_v3MaxL.z);
	//TRB
	cornerRealigned[7] = m_v3MaxL;

	//Globalize Them
	for (int i = 0; i < 8; ++i) {
		cornerRealigned[i] = vector3(m_m4ToWorld * vector4(cornerRealigned[i], 1.0f));
	}

	//Identify the max and min as the first corner
	m_v3MaxG = vector3(std::numeric_limits<float>().min());
	m_v3MinG = vector3(std::numeric_limits<float>().max());

	//Find max and min
	for (int i = 0; i < 8; ++i)
	{
		if (m_v3MaxG.x < cornerRealigned[i].x)
			m_v3MaxG.x = cornerRealigned[i].x;

		if (m_v3MinG.x > cornerRealigned[i].x) 
			m_v3MinG.x = cornerRealigned[i].x;

		if (m_v3MaxG.y < cornerRealigned[i].y) 
			m_v3MaxG.y = cornerRealigned[i].y;

		if (m_v3MinG.y > cornerRealigned[i].y) 
			m_v3MinG.y = cornerRealigned[i].y;

		if (m_v3MaxG.z < cornerRealigned[i].z) 
			m_v3MaxG.z = cornerRealigned[i].z;

		if (m_v3MinG.z > cornerRealigned[i].z) 
			m_v3MinG.z = cornerRealigned[i].z;
	}
