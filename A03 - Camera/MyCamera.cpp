...
//Moves forward based on the target point
void MyCamera::MoveForward(float a_fDistance)
{
	//Adds the vector with the magnitude of a_fDistance from your position to the target to each one of the vectors
	vector3 toAdd = glm::normalize(m_v3Target - m_v3Position) * a_fDistance;
	m_v3Position += toAdd;
	m_v3Target += toAdd;
	m_v3Above += toAdd;
}
//Moves vertically in world space
void MyCamera::MoveVertical(float a_fDistance){
	//Adds the vector with the magnitude of a_fDistance from your position to the above point to each one of the vectors
	vector3 toAdd = glm::normalize(m_v3Above - m_v3Position) * a_fDistance;
	m_v3Position += toAdd;
	m_v3Target += toAdd;
	m_v3Above += toAdd;
}
//Moves sideways based on the cross product of the vector from position to target and up vector
void MyCamera::MoveSideways(float a_fDistance) {
	//Adds the vector with the magnitude of a_fDistance from your position to the right point to each one of the vectors
	vector3 toAdd = glm::normalize(glm::cross((m_v3Above - m_v3Position), (m_v3Target - m_v3Position))) * a_fDistance;
	m_v3Position += toAdd;
	m_v3Target += toAdd;
	m_v3Above += toAdd;
}
//Rotates the camera based on given angles
void MyCamera::RotateCamera(float a_fAngleX, float a_fAngleY) {
	//Rotates the camera based on the angle
	m_v3Target = glm::rotate(m_v3Target, a_fAngleY, vector3(0, 1, 0));
	m_v3Target = glm::rotate(m_v3Target, -a_fAngleX, glm::cross((m_v3Target - m_v3Position), vector3(0, 1, 0)));
}
